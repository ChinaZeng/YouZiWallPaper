package com.youzi.framework.uiimpl01.loading;
...
public class LoadingHelperImpl implements ILoadingHelper {
    ...
    private ILoadingViewInterceptor loadingViewInterceptor;
    ...
    public static synchronized LoadingHelperImpl with(View view) {
        ...
        if (tag != null && tag instanceof LoadingHelperImpl) {
            ...
            view.setTag(R.id.uiimpl_id_tag_loading_helper, loadingHelper);
    ...
    public ILoadingHelper showLoading() {
        synchronized (helper.getView()) {
            state = State.LOADING;
            View view = helper.inflate(R.layout.uiimpl_layout_loading_state_loading);
            if (loadingViewInterceptor != null) {
                view = loadingViewInterceptor.onInflateLoadingView(view, state);
            }
    ...
    public ILoadingHelper showEmpty() {
        synchronized (helper.getView()) {
            state = State.EMPTY;
            View view = helper.inflate(R.layout.uiimpl_layout_loading_state_load_no_data);
            if (loadingViewInterceptor != null) {
                view = loadingViewInterceptor.onInflateLoadingView(view, state);
            }
    ...
    public ILoadingHelper showNoNetwork() {
        synchronized (helper.getView()) {
            ...
            View view = helper.inflate(R.layout.uiimpl_layout_loading_state_no_network);
            if (loadingViewInterceptor != null) {
                view = loadingViewInterceptor.onInflateLoadingView(view, state);
            }
    ...
    public ILoadingHelper showError() {
        synchronized (helper.getView()) {
            state = State.ERROR;
            View view = helper.inflate(R.layout.uiimpl_layout_loading_state_load_failed);
            if (loadingViewInterceptor != null) {
                view = loadingViewInterceptor.onInflateLoadingView(view, state);
            }
    ...
    public ILoadingHelper showLogin() {
        synchronized (helper.getView()) {
            ...
            View view = helper.inflate(R.layout.uiimpl_layout_loading_state_load_need_login);
            if (loadingViewInterceptor != null) {
                view = loadingViewInterceptor.onInflateLoadingView(view, state);
            }
    ...
    @Override
    public ILoadingHelper showView(View view) {
        synchronized (helper.getView()) {
            state = State.CUSTOM_VIEW;
            helper.showLayout(view);
            notifyStateChange();
            return this;
        }
    ...
    @Override
    public ILoadingHelper setViewInterceptor(ILoadingViewInterceptor interceptor) {
        this.loadingViewInterceptor = interceptor;
        return this;
    }
    ...
    @Override
    public ILoadingHelper setStateChangeListener(OnStateChangeListener listener) {
    ...
    private void notifyStateChange() {
        if (this.onStateChangeListener != null) {
            this.onStateChangeListener.onStateChanged(this, getState(), getCurrentView());
        }
    }
}
